...
func (r *Neo4jRepository) ensureDriver() (neo4j.Driver, error) {
	...
	if r.cfg.URI == "" {
		return nil, fmt.Errorf("no Neo4j URI configured — set LOM_NEO4J_URI or neo4j.uri in your config file")
	}
	if r.cfg.Username == "" {
		return nil, fmt.Errorf("no Neo4j username configured — set LOM_NEO4J_USERNAME or neo4j.username in your config file")
	}
