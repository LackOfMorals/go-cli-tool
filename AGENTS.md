...
- `LOM_SHELL_ENABLED` env var is handled automatically by viper's AutomaticEnv via the `shell.enabled` → `LOM_SHELL_ENABLED` key remapping (prefix NCTL, `.` → `_`)
...
- Viper env prefix is now `NCTL` (was `CLI`), so config env vars are `LOM_NEO4J_URI`, `LOM_AURA_CLIENT_SECRET`, `LOM_SHELL_ENABLED`, etc. (renamed in task-003).
